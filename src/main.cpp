...
// ── Embedded font ─────────────────────────────────────────────
// font_cousine.h is auto-generated by the build workflow via:
//   xxd -i Cousine-Regular.ttf > src/font_cousine.h
// It provides:
//   static const unsigned char cousine_ttf_data[];
//   static const unsigned int  cousine_ttf_size;
//
// Cousine is a monospace font from Google / Steve Matteson with
// retro terminal character, full Latin + General Punctuation
// coverage (em-dash U+2014, bullet U+2022, etc.).
// Ships inside Dear ImGui repo at misc/fonts/Cousine-Regular.ttf.
// By embedding it we need zero external files at runtime.
#include "font_cousine.h"
...
// ─── action bar ──────────────────────────────────────────────
// FIX: all text and icon positions use cy + measured font height
// so nothing drifts regardless of which font/size is loaded.
static void DrawActionBar(ImDrawList* dl,ImVec2 pos,float w)
{
    ...
    float cy  = pos.y + h * .5f;   // vertical centre of the bar
    float x   = pos.x + 12.f;
    float btnH = 30.f;
    float by   = cy - btnH * .5f;  // top of every pill button

    // Pre-measure the label height once — all labels use g_font13 @ 13px
    float textH = TS("X").y;       // height of a single line of body text
    float textY = cy - textH * .5f; // baseline-Y that centres text in the bar

    // Icon half-size: icon radius = textH * .55 keeps icon optically level with text
    float iconR = textH * .55f;
    ...
    // ── Like pill ─────────────────────────────────────────────
    {
        ...
        ImVec2 cs  = TS(cnt);
        float sepW = 1.f;
        float gap  = 6.f;       // gap between icon edge and separator / text
        float pillW = iconR * 2.f + gap + sepW + gap + cs.x + 16.f;
        ImGui::SetCursorScreenPos({x, by});
        ...
        float iconCX = x + 8.f + iconR;
        IconThumbUp(wdl, {iconCX, cy}, iconR, C_ACCENT);
        // separator
        float sepX = iconCX + iconR + gap;
        wdl->AddLine({sepX, by + 6.f}, {sepX, by + btnH - 6.f}, C_ACCENT_LINE, 1.f);
        // count text, vertically centred
        Txt(wdl, {sepX + gap, textY}, C_ACCENT, cnt);
        x += pillW + 4.f;
    }
    ...
    // ── Dislike pill ──────────────────────────────────────────
    {
        ...
        ImVec2 ls   = TS(lbl);
        float pillW = iconR * 2.f + 8.f + ls.x + 16.f;
        ImGui::SetCursorScreenPos({x, by});
        ...
        Txt(wdl, {iconCX + iconR + 8.f, textY}, hov ? C_TEXT : C_TEXT_MUTED, lbl);
        x += pillW + 4.f;
    }
    ...
    // ── Share pill ────────────────────────────────────────────
    {
        ...
        Txt(wdl, {iconCX + iconR + 8.f, textY}, ic, lbl);
        x += pillW + 4.f;
    }
    ...
    ImVec2 ms = TS(meta);
    Txt(wdl, {pos.x + w - ms.x - 12.f, textY}, C_TEXT_FAINT, meta);
}
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE,LPSTR,int nCmdShow)
{
    ...
    // ── Font loading ──────────────────────────────────────────
    // Cousine-Regular.ttf is embedded at compile time via font_cousine.h
    // (auto-generated by the build workflow using xxd -i).
    // cousine_ttf_size == 0 means the stub header is present (local dev
    // without running the workflow), in which case we fall back to the
    // built-in Proggy Clean (ASCII only).
    //
    // AddFontFromMemoryTTF transfers ownership to ImGui — no free() needed.

    if(cousine_ttf_size > 1) {
        // Make a mutable copy — ImGui may modify the buffer
        void* buf13 = IM_ALLOC(cousine_ttf_size);
        memcpy(buf13, cousine_ttf_data, cousine_ttf_size);
        ImFontConfig c13;
        c13.FontDataOwnedByAtlas = true;
        c13.GlyphRanges = g_glyph_ranges;
        g_font13 = io.Fonts->AddFontFromMemoryTTF(buf13, (int)cousine_ttf_size, 13.f, &c13, g_glyph_ranges);

        void* buf16 = IM_ALLOC(cousine_ttf_size);
        memcpy(buf16, cousine_ttf_data, cousine_ttf_size);
        ImFontConfig c16;
        c16.FontDataOwnedByAtlas = true;
        c16.GlyphRanges = g_glyph_ranges;
        g_font16 = io.Fonts->AddFontFromMemoryTTF(buf16, (int)cousine_ttf_size, 16.f, &c16, g_glyph_ranges);
    }

    // Fallback: try filesystem fonts (useful when building locally without xxd)
    if(!g_font13) {
    ...
    // Last resort: built-in Proggy Clean (ASCII only)
    if(!g_font13) {

...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE,LPSTR,int nCmdShow)
{
    ...
    // ── Font loading ──────────────────────────────────────────
    // Strategy: embed Electrolize at compile time via font_electrolize.h
    // (generated by the CI workflow downloading from Google Fonts and
    // running xxd -i on the .ttf).
    // Electrolize is licensed under SIL OFL 1.1 — free to embed.
    //
    // font_electrolize.h exposes:
    //   const unsigned char electrolize_regular_ttf[];
    //   const unsigned int  electrolize_regular_ttf_len;
    //
    // Fallback chain (if header not real / font load fails):
    //   1. Electrolize-Regular.ttf  (next to exe)
    //   2. C:\Windows\Fonts\segoeui.ttf  (Vista+)
    //   3. C:\Windows\Fonts\tahoma.ttf   (XP+)

...
// ── Embedded font ──────────────────────────────────────────────
// font_proggy.h is auto-generated at build time by the workflow /
// build script using:  xxd -i imgui/misc/fonts/ProggyVector.ttf
// It defines:  extern const unsigned char proggy_vector_ttf[];
//              extern const unsigned int  proggy_vector_ttf_len;
// This means no external .ttf file is needed at runtime — the font
// is baked into the EXE and works on XP through Windows 11.
#if __has_include("font_proggy.h")
  #include "font_proggy.h"
  #define HAS_PROGGY_EMBED 1
#else
  #define HAS_PROGGY_EMBED 0
#endif
...
// ─── action bar ──────────────────────────────────────────────
// FIX: all pill content (icon + label) uses cy as vertical anchor.
// Icon center = {iconCX, cy}, text top = cy - textH*.5f — perfectly centred.
static void DrawActionBar(ImDrawList* dl,ImVec2 pos,float w)
{
    const float h     = 46.f;
    const float btnH  = 30.f;
    const float iconR = 6.5f;   // half-size of icon drawing area
    const float padLR = 10.f;   // horizontal padding inside pill
    const float gap   = 4.f;    // pill-to-pill gap
    ...
    const float by  = cy - btnH * .5f;   // pill top
    float x         = pos.x + 12.f;

    ImDrawList* wdl = ImGui::GetWindowDrawList();

    // ── Like pill (icon + divider + count) ──────────────────
    {
        const char* cnt = "248K";
        ImVec2 cs = TS(cnt);
        // pill width = left-pad + icon + gap + divider + gap + text + right-pad
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE,LPSTR,int nCmdShow)
{
    ...
    // ── Font loading ──────────────────────────────────────────
    // Strategy: embed ProggyVector at compile time via font_proggy.h
    // (generated by xxd -i during the build step).
    // This guarantees the retro look on every machine without needing
    // any external .ttf at runtime — works on XP through Windows 11.
    //
    // font_proggy.h exposes:
    //   const unsigned char proggy_vector_ttf[];
    //   const unsigned int  proggy_vector_ttf_len;
    //
    // Fallback chain (if header not generated / font load fails):
    //   1. Cousine-Regular.ttf  (next to exe)
    //   2. C:\Windows\Fonts\cour.ttf   (XP+)
    //   3. C:\Windows\Fonts\consola.ttf
    //   4. C:\Windows\Fonts\arial.ttf
    //   5. AddFontDefault() — Proggy Clean, ASCII only
...
#if HAS_PROGGY_EMBED
    {
        ...
        g_font13 = io.Fonts->AddFontFromMemoryTTF(
            (void*)proggy_vector_ttf, (int)proggy_vector_ttf_len,
    ...
    // File-based fallback (covers dev machines and XP where embed may be absent)
    if(!g_font13){
        ...
        for(int fi = 0; candidates[fi]; fi++){
            FILE* f = fopen(candidates[fi], "rb");
            if(!f) continue;
            fclose(f);
            ImFontConfig c13; c13.GlyphRanges = g_glyph_ranges;
            g_font13 = io.Fonts->AddFontFromFileTTF(candidates[fi], 13.f, &c13, g_glyph_ranges);
            ImFontConfig c16; c16.GlyphRanges = g_glyph_ranges;
            g_font16 = io.Fonts->AddFontFromFileTTF(candidates[fi], 16.f, &c16, g_glyph_ranges);
            break;
        }
    }
